...
class InputData:
    ...
    def create_mock(self, batch:int, seq_len:int, vocab_size:int)->None:
        ...
        for _ in range(batch):
            batch_data = [np.random.randint(vocab_size) for _ in range(seq_len)]
            self.all_data.append(batch_data)
            self.batch_seqlen.append(seq_len)

    def get_padded_input(self, pad_id:int)->None:
        padded_input = np.full((self.input_total_batch, self.max_seq_len), pad_id)
...
class GptConfig:
    ...
    def load_model_config(self, config:Config):
        ...
        # self.parseGptConfig(config.entries()) # FIXME:后面再看看这个是不是必要的
        # FROM include/bt_gpt.h
        ...
        self.filter_data_type = config.get("filter_data_type","CNNL_DTYPE_HALF")
        self.context_mha_compute_data_type = config.get("context_mha_compute_data_type","CNNL_DTYPE_HALF")
        self.filter_bitwidth = config.get_int("filter_bitwidth",0)
        self.quantize_layout = config.get("quantize_layout","CNNL_QUANTIZE_NONE")
        self.is_embedding_weight_quantized = config.get_bool("is_embedding_weight_quantized", False)
        self.norm_type = config.get("norm_type","CNNL_TRANSFORMER_LAYERNORM")
        self.lnres_mode = config.get("lnres_mode","CNNL_TRANSFORMER_PRE_LAYERNORM_INSIDE_RESIDUAL")
        ...
        self.mha_mul_factor_after_qk = config.get_bool("mha_mul_factor_after_qk", False)
        self.position_embedding_type = config.get("position_embedding_type","CNNL_ATTN_FOLD_ROTARY_EMBEDDING")
        self.rotary_pos_emb_dim = config.get_int("rotary_pos_emb_dim",0)
        self.preload_size = config.get_int("preload_size",0)
        self.context_only_last_logit = config.get_bool("context_only_last_logit",True)
        self.is_individual_token_offset = config.get_bool("is_individual_token_offset",False)
        ...
        self.position_embedding_seq_len_q = config.get_int("position_embedding_seq_len_q",0)
        self.position_embedding_seq_len_k = config.get_int("position_embedding_seq_len_k",0)
        # self.cache_memory_len = config.get_int("cache_memory_len",0) #FIXME: C++  实现没有去获取这个参数
    def __str__(self):
        params = [
    ...
    def load_input_config(self, config:Config):
        ...
        if not config.has_key("input_path") or config.get("input_path") == "":
            ...
            if config.has_key("mock_input_batch_size") and config.has_key("mock_input_seq_len"):
                self.mock_input_batch_size = config.get_int("mock_input_batch_size")
                self.mock_input_seq_len = config.get_int("mock_input_seq_len")
            else:
                raise Exception("mock_input_batch_size and mock_input_seq_len must be set when input_path is not set")
            self.input_data_.create_mock(self.mock_input_batch_size, self.mock_input_seq_len, self.model_config_.get_int("vocab_size"))
        else:
            ...
            if config.has_key("mock_input_batch_size") or config.has_key("mock_input_seq_len"):
                print("mock_input_batch_size and mock_input_seq_len will be ignored when input_path is set")
            for i in range(self.input_data_.input_total_batch):
                for j in range(len(self.input_data_.all_data[i])):
                    ...
                    if word < 0:
                        raise ValueError(f"input data batch {i} token {j} is negative: {word}")
                    if word > self.vocab_size:
                        raise ValueError(f"input data batch {i} token {j} is out of vocab: {word}, vocab_size: {self.vocab_size}")
        ...
        self.position_embedding_seq_len_q = config.get_int("position_embedding_seq_len_q",0)
        self.position_embedding_seq_len_k = config.get_int("position_embedding_seq_len_k",0)
        self.cache_memory_len = config.get_int("cache_memory_len",0) # FIXME: C++ 实现没有读这个参数，可能是个BUG
        ...
        if not self.position_embedding_seq_len_q or not self.position_embedding_seq_len_k \
            ...
            print("position_embedding_seq_len_q/k not set, assume same as cache_memory_len")

        self.once_batch_size = min(self.once_batch_size, self.input_data_.input_total_batch)
        self.cache_memory_len = max(self.cache_memory_len, self.input_data_.max_seq_len + self.output_size + 1)
        self.position_embedding_seq_len_q = max(self.cache_memory_len, self.position_embedding_seq_len_q)
        self.position_embedding_seq_len_k = max(self.cache_memory_len, self.position_embedding_seq_len_k)
